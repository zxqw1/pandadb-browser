                ...
                <el-row>
                    ...
                    <el-col style="margin-top: 10px;">
                        <el-table :data="tableData" style="width: 100%" border row-key="key">
                            ...
                            <el-table-column property="progress" fixed="right" label="备份进度">
...
const confirmBackup = async (formRef) => {
    await formRef.validate(async (valid, fields) => {
        if (valid) {
            ...
            if (form.value.planRunTime !== null) {
            ...
            const dataBackupQuery = {
                ...
                'type': type.value,
                'planRunTime': (form.value.planRunTime === "null" || form.value.planRunTime === null) ? "" : form.value.planRunTime,
...
const Backup = async (record) => {
    ...
    const progressData = await getManageInfo(progressUrl, "POST", JSON.stringify(progressquery))
    ...
    await formRef2.validate(async (valid, fields) => {
        if (valid) {
            ...
            const dataBackupQuery = {
                ...
                'planRunTime': (form.value.planRunTime === "null" || form.value.planRunTime === null) ? "" : reform.value.planRunTime,
